...
https://gist.github.com/bpj/a10e8373ca82bf5678db80abee2f43ef
...
local function markup_inlines (inlines)
  -- return pandoc.Span(inlines, {['custom-style']='highlight'})
    ...
    if inline.tag == 'Str' then
      if not markup then
        ...
        if first then
          ...
          if selfclosing then
            result:insert(pandoc.RawInline('latex', '\\myhighlight'))
